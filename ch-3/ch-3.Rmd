...
Notes:
1. relationships between alternatives (choices), agents, and observers
2. e.g., stay or move
For alternatives: cost_{si}, time_{si}
For agents: income_{i}
For observers: random utility
U_{si} (net utility) = V_{si} (systematic utility) + e_{si} (random utility)

--> Probability_s = P(e_{mi} - e_{si} < V_{si} - V_{mi})


PDF: 1. P(X) >= 0; 2. F(X) = 1
CDF

Choice Models 和 Split Models (比如logit) 是不一样的
