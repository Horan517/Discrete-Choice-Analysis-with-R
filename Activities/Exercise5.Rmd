...
```{r ch05-create-matrix-to-predict, include=FALSE}
mc_commute_predict <- mc_commute_long[1:12,]
```

```{r ch05-set-values-for-predictions, include=FALSE}
...
```{r ch05-median-commute-time, include=FALSE}
median(mc_commute_predict$sidewalk_density, 
       na.rm = TRUE)
```

```{r ch05-set-time-for-prediction, include=FALSE}
mc_commute_predict$time <- 22.63
```

```{r ch05-predict-probabilities-model-2, include=FALSE}
probs <- predict(model2,
                 newdata = mc_commute_predict)
```

```{r ch05-reshape-table, include=FALSE}
...
```{r ch05-probability-plot, echo=FALSE}
probs %>%
...
```{r ch05-summary-model-3-change-of-reference, results="asis"}
# Note: use chunk option results="asis" to display latex output in pdf
model3 <- mlogit(f3,
                 mc_commute_long, 
...
probs2 <- predict(model3,
                 newdata = mc_commute_predict)
```

```{r ch05-reshape-table}
probs2 <- data.frame(time = seq(from = 20,
                               to = 40,
                               by = 10),
                    probs2) %>% 
  # Pivot longer all columns _except_ `sidewalk_density`
...
```{r ch05-likelihood-ratio-test-2-3}
lrtest(model2,
       model3)
```
