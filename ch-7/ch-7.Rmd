...
两个lambda都不接近1 --> 说明组内的variable之间高度相关，并不是IIA --> 不能是multi-nomial logit
...
# nested model is way better to choose!!!
...
# no diff between the "single lambda" value
...
results show that the substitution effects within the nests are the same
...
question discussion: the multiplication of `mean_ic == "ec"` part are actually correct. 
...
can compare the ultinomial logit model of elasticities below with the nested model above.
