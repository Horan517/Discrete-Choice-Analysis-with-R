...
mixl_t <- gmnl(choice ~ vcost + travel + wait | 1,
             data = TM,
             model = "mixl", 
             ranp = c(travel = "n"), # "n" means normal distribution
             R = 50)
...
# 0th quartile
mu_w2.0 <- coef(mixl_w2)['wait'] + coef(mixl_w2)["wait:income"] * q[1]
...
# second quartile
mu_w2.2 <- coef(mixl_w2)['wait'] + coef(mixl_w2)["wait:income"] * q[3]
...
# last quartile
mu_w2.4 <- coef(mixl_w2)['wait'] + coef(mixl_w2)["wait:income"] * q[5]
sigma_w2.1 <- coef(mixl_w2)['sd.wait']
...
df_w2.0 <- data.frame(x =seq(from = -0.6, 
                             to = 0.2, 
                             by = 0.005)) %>% 
  mutate(normal = dnorm(x, 
                        mean = mu_w2.0, 
...
df_w2.2 <- data.frame(x =seq(from = -0.6, 
                             to = 0.2, 
                             by = 0.005)) %>% 
...
df_w2.4 <- data.frame(x =seq(from = -0.6, 
                             to = 0.2, 
                             by = 0.005)) %>% 
