...
time.Active.clean <- mc_mode_choice %>% # Pipe data frame `mc_mode_choice`
  select(c("timecycle", # *Select columns* from the data frame that was piped
           "timewalk")) %>% 
  filter(timecycle != 100000 & timewalk != 100000) # Filter observations that are _not_ 100000 # *selecting rows* based on conditions 
```
...
ggplot(aes(x = choice, 
  ...
  geom_boxplot()
```
- Categorical (nominal, ordinal), Quantitative (intervals, ratio)
- 以后针对categorical variables最好还是code成原本的text (nominal - male/female, ordinal - satisfactory levels).
- 对于所有categorical variables, 都可以处理成dummy variables (比如 5-point Likert scale可以处理成4个变量: SA, A, D, SD, 如果都是0就是Neutral)：这两者的区别是什么？



## 1.	Deﬁne “model”.

Translation into maths/symbols of a system
simplifying representation of reality
relationships between different parts based on hypotheses
...
There are nine variables and only "choice" is a nominal categorical variable as "factor", while the other eight variables are all quantitative variables in ratio scale.
...
```{r ch01-exercise-q6, results='hide'}
Mode %>%
  summary()
```
